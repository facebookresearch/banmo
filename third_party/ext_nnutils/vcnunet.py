...
#from nnutils.pointnet_utils import PointNetSetAbstraction,PointNetFeaturePropagation
...
class pspnet(nn.Module):
    ...
    def __init__(self, is_proj=True,groups=1):
        ...
        if self.is_proj:
            ...
            self.proj2 = nn.Conv2d(64, 64 ,1, padding=0,stride=1)
...
class SurfaceMatchNet(nn.Module):
    def __init__(self, nfeat, ishuman):
        ...
        if nfeat==64:
            ...
            if ishuman:
                # human
                config_fpath = 'third_party/densepose/configs/cse/densepose_rcnn_R_101_FPN_DL_soft_s1x.yaml'
                weights_fpath = 'https://dl.fbaipublicfiles.com/densepose/cse/densepose_rcnn_R_101_FPN_DL_soft_s1x/250713061/model_final_1d3314.pkl'
            else:
                # animals
                config_fpath = 'third_party/densepose/configs/cse/densepose_rcnn_R_50_FPN_soft_animals_CA_finetune_4k.yaml'
                weights_fpath = 'https://dl.fbaipublicfiles.com/densepose/cse/densepose_rcnn_R_50_FPN_soft_animals_CA_finetune_4k/253498611/model_final_6d69b7.pkl' 
            cfg = get_cfg()
        ...
        self.featnerf = getattr(cmlps, 'CondNeRFModel')(
                    ...
                    out_channel=nfeat,
                    codesize=0)
        self.encode_position_fn = cmlps.get_embedding_function(
    ...
    def forward(self, img, masks, verts, faces, transform, detach=False):
        ...
        def dpinf(model, img, transform):
            img = ((img.clone()*torch.Tensor(transform.std).cuda()[None,:,None,None]) +\
                            torch.Tensor(transform.mean).cuda()[None,:,None,None])*255
            img = torch.stack([(x - self.featnet.pixel_mean) / self.featnet.pixel_std for x in img])
            ...
            features_dp = model.roi_heads.densepose_pooler(features, [pred_boxes]*img.shape[0]).detach()
            ...
            feat = densepose_predictor_outputs.embedding
            feat = feat/5
            return feat
        ...
        if self.nfeat==64:
            ...
            feat = dpinf(self.featnet, img, transform)
        ...
        ## pointnet, bhw, c, n -> bhw, n
        #pointcorr = pointcorr.permute(0,3,4,2,1).reshape(-1,feat.shape[1],npts) # bhw,c,n
        #pointcorr = torch.cat([pts[:,None].repeat(1,h*w//16,1,1).view(-1,npts,3).permute(0,2,1), pointcorr], 1)
        #pointcorr = self.pointnet(pointcorr)
        #pred = torch.softmax(1000*pointcorr,-1).view(bs,-1,npts,1) * pts[:,None]
        
        # corr
        #pointcorr = (feat/feat.norm(2,1)[:,None])[:,None] *\
        #            (feat_nerf/feat_nerf.norm(2,-1)[:,:,None])[:,:,:,None,None] # b,n,c,h,w
        ...
        # 3d-2d mapping
        masks = F.interpolate(masks[:,None].float(), (hf, wf))
        pointcorr_2d = pointcorr.clone()
        #pointcorr_2d[masks[:,0,:,:,None].repeat(1,1,1,npts)<=0] = pointcorr_2d.min()

        ipred = pointcorr_2d.view(bs,-1,npts)
        ...
        with torch.no_grad():
            ...
            fberr = (-10*fberr).sigmoid() * 2
            #fberr = fberr / fberr[masks.repeat(rep,1,1,1)>0].mean()
            #import cv2; cv2.imwrite('tmp/0.png', np.asarray(reproj_err.cpu())*100)
            #pred = fberr
            #pred[0,0][masks[0,0]<=0] = 0
            #pred = ((pred-0.5)*2).repeat(1,3,1,1)
            fberr = F.interpolate(fberr, (h,w), mode='bilinear')

        # uncertainty
        #pred = torch.softmax(pointcorr,-1)
        #pred = -(pred * torch.log(pred+ 1e-9)).sum(-1)[:,None].repeat(1,3,1,1)/5
        #pred = 1-(pred).max(-1)[0][:,None].repeat(1,3,1,1)

        ## 2d-3d mapping vis
        #import cv2; import trimesh
        #from matplotlib import cm
        #viridis = cm.get_cmap('jet', 12)
        #colors = pointcorr[0,10,39]
        #colors = colors-colors.mean()
        #colors = colors / colors.std()
        #colors = (colors-colors.min())/(colors.max()-colors.min())
        #colors = viridis(colors.cpu())[:,:3]
        #trimesh.Trimesh(verts.cpu()[0],faces.cpu()[0],vertex_colors=colors).export('/data/gengshay/0.ply')
        #cv2.imwrite('/data/gengshay/0.png', np.asarray(feat.std(1)[0].cpu())*255)

        ## 3d-2d mapping vis
        #pdb.set_trace()
        #for i in range(npts):
        #    colors = pointcorr_2d[0,:,:,i].view(-1)
        #    colors = colors-colors.mean()
        #    colors = colors / colors.std()
        #    colors = (colors-colors.min())/(colors.max()-colors.min())
        ...
        return pred, ipred, pts, feat, fberr
