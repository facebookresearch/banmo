...
def nerf_mesh(key,
  ...
  init_rays_dict = {
      ...
      'query_xyz': jnp.ones((batch_size, 3), jnp.float32)
  }
...
class NerfModel_mesh(nn.Module):
  """Nerf NN Model with both coarse and fine MLPs.
  ...
  def __call__(
    ...
    # marching cubes
    import pdb
    points = rays_dict['query_xyz'][:,None]
    metadata = rays_dict['metadata']
    if self.use_warp and use_warp:
      metadata_channels = self.num_warp_features if metadata_encoded else 1
      warp_metadata = jnp.broadcast_to(
    ...
    condition_inputs = []
    if self.use_viewdirs:
      viewdirs = np.zeros((points.shape[0],3))
      viewdirs_embed = self.viewdir_encoder(viewdirs)
      condition_inputs.append(viewdirs_embed)
    ...
    if self.use_appearance_metadata:
      if metadata_encoded:
        ...
        appearance_code = self.appearance_encoder(appearance)
      condition_inputs.append(appearance_code)
    ...
    condition_inputs = (
        jnp.concatenate(condition_inputs, axis=-1)
        if condition_inputs else None)
    condition_inputs_rep = np.repeat(condition_inputs[:1],points_embed.shape[0],axis=0)
    output = self.nerf_coarse(points_embed, condition=condition_inputs_rep)
    #output = self.nerf_fine(points_embed, condition=condition_inputs_rep)
    out = {'alpha': self.sigma_activation(output['alpha'])}
    return out
