...
//template <typename scalar_t>
//__device__ __forceinline__ scalar_t backward_sample_texture2(const scalar_t grad_color, const scalar_t *w, const int R, const int k, const int texture_sample_type) {
//    scalar_t grad_texture_k=0;
//    if (texture_sample_type == 0) { // sample surface color with resolution as R
//        const int w_x = w[0] * R;
//        const int w_y = w[1] * R;
...
//            if (k == w_y * R + w_x) {
//                grad_texture_k = grad_color;
//                printf("%d,%d,%d,%f,",w_x,w_y,w_y * R + w_x,grad_texture_k*10000000);
//            }
//        } else {
//            if (k == (R - 1 - w_y) * R + (R - 1 - w_x)) {
//                grad_texture_k = grad_color;
        ...
        if (func_id_rgb == 1 && (check_face_frontside(face) || double_side)) { // Softmax (Z * D)
            ...
            for (int k = 0; k < 3; k++) {
                ...
                for (int j = 0; j < texture_size; j++) {
                    ...
                    //if (i==32896&k==0&texture_res==2){
                    //    const scalar_t grad_t = backward_sample_texture2(grad_soft_color_k, w, texture_res, j, texture_sample_type);
                    //    printf("%d,%d,%d,%d,%.2f,%.2f... ",xi,yi,fn,j,10000000*grad_soft_color_k, 10000000*grad_t);
                    //    atomicAdd(&grad_texture[3 * j + k], zp_softmax * grad_t);
                    //}
                }
