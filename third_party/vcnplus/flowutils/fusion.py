...
class TSDFVolume:
    ...
    if self.gpu_mode:
      ...
      self._cuda_src_mod = SourceModule("""
        __global__ void integrate(float * tsdf_vol,
          ...
          float obs_weight_im = weight_im[pixel_y*im_w+pixel_x];
          //printf("%f", obs_weight);
          //printf("%f", obs_weight_im);
          obs_weight = obs_weight_im;
  ...
  def integrate(self, color_im, depth_im, cam_intr, cam_pose, weight_im, obs_weight=1.):
    ...
    if self.gpu_mode:  # GPU mode: integrate voxel volume (calls CUDA kernel)
      for gpu_loop_idx in range(self._n_gpu_loops):
        self._cuda_integrate(self._tsdf_vol_gpu,
                            ...
                            cuda.InOut(weight_im.reshape(-1).astype(np.float32)),
